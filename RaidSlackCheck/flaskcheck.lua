...
--txt main
	...
	rscmaintxt1:SetFont(GameFontNormal:GetFont(), rscfontsset[2], 'MONOCHROME')
...
--txt flaskinfo
	...
	rscflaskout:SetFont(GameFontNormal:GetFont(), rscfontsset[2], 'MONOCHROME')
	rscflaskout:SetText(" ")
...
--txt flask time info
	...
	rscflaskout2:SetFont(GameFontNormal:GetFont(), 14, 'MONOCHROME')
...
--txt autocheck
	...
	tt2:SetFont(GameFontNormal:GetFont(), 12, 'MONOCHROME')
...
--txt manual report
	...
	tt22:SetFont(GameFontNormal:GetFont(), 12, 'MONOCHROME')
...
--txt not in some zones
	...
	tt3:SetFont(GameFontNormal:GetFont(), rscfontsset[2], 'MONOCHROME')
...
--3 chechbox and text
	...
	for hhb=1,4 do
		local rsccheckboxhhb = CreateFrame("CheckButton", nil, rscframetoshowall4, "InterfaceOptionsCheckButtonTemplate")
	...
	t1:SetFont(GameFontNormal:GetFont(), rscfontsset[1], 'MONOCHROME')
...
--7 checkbox
	...
	local rsccheckbox1 = CreateFrame("CheckButton", nil, rscframetoshowall4, "InterfaceOptionsCheckButtonTemplate")
	...
	local rsccheckbox8 = CreateFrame("CheckButton", nil, rscframetoshowall4, "InterfaceOptionsCheckButtonTemplate")
	...
	local rsccheckbox9 = CreateFrame("CheckButton", nil, rscframetoshowall4, "InterfaceOptionsCheckButtonTemplate")
...
for as=1,5 do
	...
	rscflaskt6:SetFont(GameFontNormal:GetFont(), rscfontsset[1], 'MONOCHROME')
...
if rscflaskcheckb[6]==0 then
	...
	rscflaskt7:SetFont(GameFontNormal:GetFont(), rscfontsset[1], 'MONOCHROME')
...
if rscflaskcheckb[7]==0 then
	...
	rscflaskt8:SetFont(GameFontNormal:GetFont(), rscfontsset[1], 'MONOCHROME')
...
if rscflaskcheckb[8]==0 then
	...
	rscflasktfoodadd:SetFont(GameFontNormal:GetFont(), rscfontsset[1], 'MONOCHROME')
	rscflasktfoodadd:SetText(format(rscpartanonsfoodadd,"+XXX"))
	...
	rscflasktfoodadd2:SetFont(GameFontNormal:GetFont(), rscfontsset[1], 'MONOCHROME')
	rscflasktfoodadd2:SetText(format(rscpartanonsfoodadd,"+XXX"))
...
if GetNumGroupMembers() > 0 and (UnitInRaid("player") or UnitInParty("player")) then
...
if #rscwillcheckthem>0 and rscflaskcheckb[6]==1 then
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscfoodtable do
			--print ("Überprüfen Sie:",rscwillcheckthem[yy],". Sung: ",C_Spell.GetSpellInfo(rscfoodtable[uu]))
		--  if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscflasktable[uu])["name"], rscwillcheckthem[yy]) then
		-- /dump AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(87558)["name"],"Shîrîana")
		--print("rscfoodtable",rscfoodtable[uu])
		--print("rscwillcheckthem",rscwillcheckthem[yy])
		-- /dump AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(87558)["name"],rscwillcheckthem[1])
		-- /dump AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable[7])["name"],rscwillcheckthem[1])
			if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable[uu])["name"],rscwillcheckthem[yy]) then --rscwillcheckthem[yy]
			--print("TEST")
				local _, _, _, _, _, expirationTime,_, _,_,spell_id,_,_,_,_,_,stats = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable[uu])["name"],rscwillcheckthem[yy])
				--print (stats)
    ...
    if byl==0 and rscflaskcheckfoodadd and rscflaskcheckfoodadd==1 then
      for uu=1,#rscfoodtable_additional do
        if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable_additional[uu])["name"],rscwillcheckthem[yy]) then
          local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable_additional[uu])["name"],rscwillcheckthem[yy])
      ...
      for mm=1,#rscfoodtable do
        if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable[mm])["name"],rscwillcheckthem[yy]) then
          local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable[mm])["name"],rscwillcheckthem[yy])
    ...
    if byl==0 and rscflaskcheckfoodadd2 and rscflaskcheckfoodadd2==1 then
      for uu=1,#rscfoodtable_additional2 do
        if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable_additional2[uu])["name"],rscwillcheckthem[yy]) then
          local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscfoodtable_additional2[uu])["name"],rscwillcheckthem[yy])
	...
	if (byl==0 and byl~=100) then
		if (AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(430).name,rscwillcheckthem[yy]) or AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(433).name,rscwillcheckthem[yy])) then
			--персонаж ест
			byl=99
		end
	end
...
if #rscwillcheckthem>0 and rscflaskcheckb[8]==1 then
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscrunestable do
			if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscrunestable[uu]),rscwillcheckthem[yy]) then
			
			
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscrunestable[uu]),rscwillcheckthem[yy])
...
if #rscwillcheckthem > 0 then
    for yy = 1, #rscwillcheckthem do
        --print("Checking Flask for:", rscwillcheckthem[yy])
        local byl = 0
        for uu = 1, #rscflasktable do
            --print("Spell being checked rsc:", rscflasktable[uu])
            --print("Spell being checked uu:", uu)
            --print("Spell being checked:", C_Spell.GetSpellInfo(rscflasktable[uu]))

			
			--/dump AuraUtil.FindAuraByName("Fläschchen der drakonischen Gedanken", "Shîrîana")
			if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscflasktable[uu])["name"], rscwillcheckthem[yy]) then
				--local _, _, _, _, _, expirationTime, _, _, _, spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscflasktable[uu]), rscwillcheckthem[yy])
				local _, _, _, _, _, expirationTime, _, _, _, spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscflasktable[uu])["name"], rscwillcheckthem[yy])
				--local _, _, _, _, _, expirationTime, _, _, _, spell_id = C_Spell.GetSpellInfo(rscflasktable[uu])["spellID"]
				--print("Found Flask:", AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscflasktable[uu])["name"], rscwillcheckthem[yy]))
				--print("Found Flask:", C_Spell.GetSpellInfo(rscflasktable[uu])["name"], "Player:", rscwillcheckthem[yy], "Expiration Time:", expirationTime, "Spell ID:", spell_id)
				--print("Expiration Time:", expirationTime)
				--print("Spell ID:", spell_id)
				--print("rscflasktable:", rscflasktable[uu])

				if spell_id==rscflasktable[uu]  then
			  if expirationTime == nil or (expirationTime and expirationTime - GetTime() > rscminflaskgood * 60) or expirationTime == 0 then
					byl = 100
					uu = 100
					print("Flask status set to 100 for:", rscwillcheckthem[yy])
				elseif expirationTime and expirationTime - GetTime() <= rscminflaskgood * 60 and expirationTime - GetTime() > 60 then
					byl = math.ceil((expirationTime - GetTime()) / 60)
					print("Flask expiration in minutes:", byl, "for:", rscwillcheckthem[yy])
				end
				end
			end
		end
		...
		for uu2=1,#rscflasktableold do
			if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscflasktableold[uu2])["name"],rscwillcheckthem[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscflasktableold[uu2])["name"],rscwillcheckthem[yy])
...
if #rscnoflask>0 then
	for yy=1,#rscnoflask do
		...
		for uu=1,#rscelixirtable1 do
			if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscelixirtable1[uu])["name"],rscnoflask[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscelixirtable1[uu])["name"],rscnoflask[yy])
...
if #rscnoflask>0 then
	for yy=1,#rscnoflask do
		...
		for uu=1,#rscelixirtable2 do
			if AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscelixirtable2[uu])["name"],rscnoflask[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(C_Spell.GetSpellInfo(rscelixirtable2[uu])["name"],rscnoflask[yy])
...
C_ChatInfo.SendAddonMessage("RSCaddon", "a50", "Instance_CHAT")
else
C_ChatInfo.SendAddonMessage("RSCaddon", "a50", "RAID")
end
	if #rscwillnotbechecked>4 and (#rscwillnotbechecked>=#rscwillcheckthem or #rscwillnotbechecked>10) then
...
if select(3,GetInstanceInfo())==17 or IsLFGModeActive(LE_LFG_CATEGORY_LFD) or select(3,GetInstanceInfo())==18 then
C_ChatInfo.SendAddonMessage("RSCaddon", "b50", "Instance_CHAT")
else
C_ChatInfo.SendAddonMessage("RSCaddon", "b50", "RAID")
end
	for i=1,#whisper1 do
...
if #rscignorezone20[1]>0 then
	for ug=1,#rscignorezone20[1] do
		if rscignorezone20[1][ug]==curZoneID then
			rscignorezone20[2][ug]=C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name
			mojn=0
		end
	end
end
...
C_ChatInfo.SendAddonMessage("RSCaddon", "1"..strtosend.."nyyyck"..tttxt, "RAID")
end
...
if nn==1 then
	local rrr=0
	if #rscignorezone20[1]>0 then
		for i=1,#rscignorezone20[1] do
			if rscignorezone20[1][i]==curZoneID then
				rscignorezone20[2][i]=C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name
				rrr=1
			end
		end
	end
	if rrr==0 then
		table.insert(rscignorezone20[2],C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name)
		table.insert(rscignorezone20[1],curZoneID)
		out ("|cff99ffffRaidSlackCheck|r - "..rsczonereport2.."|cff00ff00"..C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name.." (ID: "..curZoneID..")|r")
	else
...
if nn==2 then
	...
	if #rscignorezone20[1]>0 then
		for i=1,#rscignorezone20[1] do
			if rscignorezone20[1][i]==curZoneID then
				rrr=1
				out ("|cff99ffffRaidSlackCheck|r - "..rsczonereport3.."|cffff0000"..C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name.."|r")
				table.remove(rscignorezone20[1],i)
				table.remove(rscignorezone20[2],i)
				i=1000
			end
		end
	end
...
if nn==3 then
	local txt=""
	if #rscignorezone20[2]>0 then
		for i=1,#rscignorezone20[2] do
			txt=txt.."|cff00ff00"..rscignorezone20[2][i].."|r"
			if #rscignorezone20[2]>i then
				txt=txt..", "
			end
		end
	end
...
table.wipe(rscignorezone20[1])
table.wipe(rscignorezone20[2])
else
--тыкни еще раз!
